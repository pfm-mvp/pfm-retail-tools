...
def api_get_report(params: List[Tuple[str, Any]], timeout: int = 90) -> Dict[str, Any]:
    base = _with_get_report_prefix(_api_base())
    full = base + "?" + urlencode(params, doseq=True)
    return _post_json(full, timeout=timeout)

def api_get_live_inside(shop_ids: List[int], source: str = "locations", timeout: int = 45) -> Dict[str, Any]:
    base = _with_get_report_prefix(_api_base())
    p: List[Tuple[str, Any]] = [("source", source)]
    p += [("data", int(i)) for i in shop_ids]
    full = base + "/report/live-inside?" + urlencode(p, doseq=True)
    return _post_json(full, timeout=timeout)
